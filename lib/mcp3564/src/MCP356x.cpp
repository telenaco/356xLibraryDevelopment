...
MCP356x::MCP356x(const MCP356xConfig& config)
    ...
    if (unslotted) { // If still true, no slot was found.
        Serial.println("Error: No free slots for MCP356x instances.");
    }

    // setupADC functionality configuration
...
bool MCP356x::usingInternalVref() {
    bool ret = false;
    if (_mcp356x_flag(MCP356X_FLAG_HAS_INTRNL_VREF)) { ret = (0 != (_reg_shadows[(uint8_t)MCP356xRegister::CONFIG0] & 0x00000040)); }
    return ret;
}
...
int8_t MCP356x::_set_scan_channels(uint32_t rval) {
    if (!_mcp356x_flag(MCP356X_FLAG_CALIBRATED)) { _channel_backup = _reg_shadows[(uint8_t)MCP356xRegister::SCAN]; }
...
/**
 * @brief Calculates the ADC conversion time based on the oversampling ratio (OSR) settings.
 *
 * This function retrieves the OSR settings from the CONFIG1 register and calculates the
 * number of DMCLK periods required for the ADC conversion. It then computes the conversion
 * time in microseconds using the DMCLK frequency. The calculated conversion time is stored
 * in the `_settling_us` member variable.
 *
...
void MCP356x::regToBinaryString(uint32_t regValue, StringBuilder* output, int bits = 32) {
    for (int i = bits - 1; i >= 0; --i) {
        output->concat((regValue & (1 << i)) ? "1" : "0");
        if (i % 4 == 0 && i != 0) { // Optional: Add a space every 4 bits for readability
            output->concat(" ");
        }
    }
}
