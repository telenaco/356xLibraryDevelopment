      ...
      if (0 == reset()) {
        ...
        if (0 == _post_reset_fxn()) {
          ...
          if (!_mclk_in_bounds()) {   // Need to detect MCLK...
            ...
            switch (det_ret) {
              case -3:    Serial.println("Class isn't ready for clock measurement."); break;
              case -2:    Serial.println("Clock measurement timed out."); break;
              case -1:    Serial.println("Problem communicating with the ADC."); break;
              case 0:     Serial.println("Clock signal within expected range determined."); break;
              case 1:     Serial.println("Clock rate out of bounds or appears nonsensical."); break;
              default:    Serial.println("Unknown return from _detect_adc_clock."); break;
            }
          }
          if (_mclk_in_bounds()) {
            Serial.println("MCLK is within bounds.");
            switch (_calibrate_offset()) {
              case 0:   ret = 0;  Serial.println("Offset calibration successful."); break;
              default:  ret = -7; Serial.println("Offset calibration failed."); break;
            }
          }
        }
        else {
