 ...
 */
 ...
 uint8_t  disp_update_rate = 5;  // Update rate in Hz for display
 uint32_t disp_update_last = 0;  // Last display update timestamp
 uint32_t disp_update_next = 0;  // Next scheduled display update
   ...
   if (Serial.available()) {
     ...
     switch (c) {
       ...
       case 'u':
         ...
         if (disp_update_rate < 0) disp_update_rate = 0;
   ...
   // Process ADC readings if autoreport is enabled
     ...
     if (autoreport) {
       if (disp_update_last <= millis()) {
         disp_update_last = millis() + (disp_update_rate > 0 ? (1000 / disp_update_rate) : 1000);
         adc0->printChannelValues(&output, true);
       }
     }
   }
